...
function Contact() {
    ...
    const [sending, setSending] = useState(false); // State to track whether email is being sent
    ...
    const sendEmail = (e) => {
        ...
        emailjs.sendForm('service_9dcjn34', 'template_lh2sx56', form.current, 'QJr7kTKXOPdy7ULGZ')
            ...
            .finally(() => {
                setSending(false); // Set sending state to false when email sending ends
            });
    };
                        ...
                        <div class="flex flex-col justify-center p-6 sm:p-8 transition-all bg-sky-50 border border-gray-100 shadow  rounded-2xl lg:p-10">
                            ...
                            <form class="space-y-6" ref={form} onSubmit={sendEmail}>
                                ...
                                <button type="submit" class="w-full px-5 py-4 text-sm md:text-base font-medium text-center text-white bg-sky-600 rounded-full hover:bg-black focus:ring-4 focus:outline-none focus:ring-blue-300">
                                    {sending ? (
                                        <img src={loadinggif} alt="loading" className="w-6 h-6 inline-block mr-2" /> // Display loading gif if sending is true
                                    ) : (
                                        "Send Message" // Otherwise display button text
                                    )}
                                </button>
